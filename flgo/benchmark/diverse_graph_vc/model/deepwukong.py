...
default_option_t = importlib.import_module(f'configs.option.model.vc_div.base').config
default_option = copy.deepcopy(default_option_t)
if os.path.exists(f'./configs/option/model/vc_div/{name}.py'):
    option_t = importlib.import_module(f'configs.option.model.vc_div.{name}').config
...
class Encoder(FModule):
    ...
    def forward(self, data):
        x, edge_index, edge_attr, batch, y = data.x, data.edge_index, data.edge_attr, data.batch, data.cwe_vc
        x = self.conv(x, edge_index)
        pooled = global_add_pool(x, batch)
        emb = self.__hidden_layers(pooled)
        return emb
