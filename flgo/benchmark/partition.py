...
class DirichletPartitionerVD(BasicPartitioner):
    """`
    Args:
        num_clients (int, optional): the number of clients
        alpha (float, optional): `alpha`(i.e. alpha>=0) in Dir(alpha*p) where p is the global distribution. The smaller alpha is, the higher heterogeneity the data is.
        ...
        index_func (func, optional): to index the distribution-dependent (i.e. label) attribute in each sample.
    """
    def __init__(self, num_clients=100, alpha=1.0, error_bar=1e-6, imbalance=0, index_func=lambda X:[xi[-1] for xi in X], minvol=1, novul_func=lambda X:[xi['cwe_cls'] for xi in X]):
        self.num_clients = num_clients
        self.alpha = alpha
        self.imbalance = imbalance
        self.index_func = index_func
        self.novul_func = novul_func
        self.minvol = minvol
        self.error_bar = error_bar
    ...
    def __call__(self, data):
        ...
        no_vul_attrs = self.novul_func(data)
        no_vul_attrs = np.array(no_vul_attrs)
        non_vul = np.where(no_vul_attrs == 0)[0]
        novul_idxs = np.random.permutation(non_vul)
        if self.imbalance == 0:
            split_arr = np.array_split(novul_idxs, self.num_clients)
        else:
            samples_per_client = self.data_imbalance_generator(self.num_clients, len(novul_idxs), self.imbalance, minvol=self.minvol)
            split_arr = []
            star = 0
            data_len = 0
            for i, num in enumerate(samples_per_client):
                data_len += len(novul_idxs[star:star+num])
                split_arr.append(novul_idxs[star:star+num])
                star = num
            assert data_len == len(novul_idxs)

        del lb_counter[-1]
        samples_per_client = self.data_imbalance_generator(self.num_clients, len(data)-len(novul_idxs), self.imbalance, minvol=self.minvol)
        lb_names = list(lb_counter.keys())
        p = np.array([1.0 * v / (len(data)-len(novul_idxs)) for v in lb_counter.values()]) # 每个类别数的占比
        lb_dict = {}
        ...
        while min(len_dist)<=self.minvol:
            ...
            local_datas[max_did] = max_d
            len_dist = [len(d) for d in local_datas]
        tmp_local = copy.deepcopy(local_datas)
        for i, local_idx in enumerate(tmp_local):
             local_datas[i].extend(split_arr[i].tolist())
        self.local_datas = local_datas
        return local_datas


class VLDiversityPartitioner(BasicPartitioner):
    ...
    def __call__(self, data):
        ...
        # cwe_class = list(set(labels))
        dpairs = [[did, lb] for did, lb in zip(list(range(len(data))), labels)]
        num = max(int(self.diversity * num_classes), 1)     # 每个客户端最多类别数
        K = num_classes
        local_datas = [[] for _ in range(self.num_clients)]
        if num == K:
            for k in range(K):
                ...
                for cid in range(self.num_clients):
                    local_datas[cid].extend(split[cid].tolist()) # 客户端随机划分
        else:
            times = [0 for _ in range(num_classes)]  # 每个类别有多少组
            contain = []                             # 每个客户端包含的类别数
