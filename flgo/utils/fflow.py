...
def read_option_from_command():
    ...
    """Training Options"""
    parser.add_argument('--a', type=str, default='fedavg')
    parser.add_argument('--m', type=str, default='codet5s')
    parser.add_argument('--b', type=str, default='bigvul_codet5s_vd')
    parser.add_argument('--t', type=str, default='RQ1')
    ...
    parser.add_argument('--model_hetero', action="store_true", default=False)
...
def gen_task(config={}, task_path:str= '', rawdata_path:str= '', seed:int=0, overwrite:bool=False):
    ...
    if type(gen_option['benchmark']['name']) is str:
        # print(gen_option['benchmark']['name'])
    ...
    if task_pipe.task_exists():
        if not overwrite:
            ...
            shutil.rmtree(task_path)
    # generate federated task  生成数据集flgo/benchmark/base.py
...
def init(task: str, algorithm, option = {}, model=None, Logger: flgo.experiment.logger.BasicLogger = None, Simulator: BasicSimulator=flgo.simulator.DefaultSimulator, scene='horizontal'):
    ...
    logger.info('args:\t{}'.format(option))
    ...
    if scene in ['horizontal', 'vertical', 'decentralized', 'hierarchical','parallel_horizontal']:
        ...
        if hasattr(model, 'init_local_module'):
            for object in objects:
                model.init_local_module(object, option) 
        if hasattr(model, 'init_global_module'):
            for object in objects:
                model.init_global_module(object, option)
...
def download_resource(root:str, name:str, type:str, overwrite:bool=False):
    ...
    resource_root = "https://github.moeyy.xyz/https://github.com/WwZzz/easyFL/raw/FLGo/resources/"
...
def zip_task(task_path:str, target_path='.', with_bmk:bool=True, ignore_names:list=['data.json', '__pycache__', 'checkpoint']):
    """
    ...
    for root, dirs, files in os.walk(task_path):
        ...
        for filename in files:
            p = os.path.join(relative_root,filename)
            if any([k in p for k in ignore_names]): continue
            zipf.write(os.path.join(root, filename), p)
    zipf.close()
    if flag:
        with open(os.path.join(task_path, 'info'), 'w') as outf:
