...
class Server(BasicServer):
    def initialize(self):
        ...
        self.num_classes = len(collections.Counter([d['cwe_vc'] for d in self.test_data]))
    ...
    def aggregate(self, models: list, cs:list, sizes_label:list):
        ...
        num_samples = np.sum(sizes_label, axis=0) # 每个类别的总数
...
class Client(BasicClient):
    def initialize(self):
        ...
        label_counter = collections.Counter([d['cwe_vc'] for d in self.train_data])
